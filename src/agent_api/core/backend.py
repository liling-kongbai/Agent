...
class Backend(QObject):
    ...
    update_chat_history_list = Signal(list)  # 更新对话历史列表
    ...
    def start(self):
        ...
        try:
            ...
            logger.debug('<start> 初始化 Agent')
            asyncio.run_coroutine_threadsafe(
                self._init_agent(), self._event_loop
    ...
    def _create_event_loop(self):
        '''创建，设置，运行事件循环。'''
        logger.debug('<_create_event_loop> 创建，设置，运行事件循环')
