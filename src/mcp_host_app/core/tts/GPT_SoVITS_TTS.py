...
class GPT_SoVITS_TTS:
    ...
    async def set_model(self):
        ...
        async with self.session.get(f'{self.base_url}/set_gpt_weights', params={'weights_path': self.config.gpt_weights_path}) as response:
        ...
        async with self.session.get(f'{self.base_url}/set_sovits_weights', params={'weights_path': self.config.sovits_weights_path}) as response:
            response.raise_for_status()
                ...
                if self.is_skip_header:
                    ...
                    if len(self.byte_buffer) >= 2:
                        ...
                        if not self.audio_stream:
                            self.audio_stream = OutputStream(samplerate=self.sample_rate, channels=self.channels, dtype='int16')
