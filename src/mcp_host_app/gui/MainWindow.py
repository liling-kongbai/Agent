...
from .views import ChatBubble
from .views import ActivityBar
from .views import Sidebar
from .views import MainContent
...
class MainWindow(QWidget):
    ...
    def _signal_connect_slot(self):
        ...
        self.mcp_host.graph_ready.connect(self.mcp_host.update_history_list)
        ...
        self.mcp_host.load_chat_history.connect(self.load_chat_history)
        self.mcp_host.update_chat_history_list.connect(self.sidebar.update_chat_history_list)
        ...
        self.mcp_host.mcp_host_finish.connect(self.mcp_host_finished)
    ...
    def update_button_state_and_switch_sidebar(self, index):
        ...
        if index == self.current_sidebar_index and current_splitter_sizes[0] > 0:
            self.splitter.setSizes([0, current_splitter_sizes[0] + current_splitter_sizes[1], current_splitter_sizes[2]])
            ...
            if not current_splitter_sizes[0] > 0:
                self.splitter.setSizes([self.last_sidebar_size, current_splitter_sizes[1] - self.last_sidebar_size, current_splitter_sizes[2]])
    ...
    def switch_panel(self, check):
        ...
        if check and current_splitter_sizes[2] == 0:
            self.splitter.setSizes([current_splitter_sizes[0], current_splitter_sizes[1] - self.last_panel_size, self.last_panel_size])
        elif not check and current_splitter_sizes[2] > 0:
            self.splitter.setSizes([current_splitter_sizes[0], current_splitter_sizes[1] + current_splitter_sizes[2], 0])
